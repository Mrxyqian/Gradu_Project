...
class TrainingStartRequest(BaseModel):
    ...
    numEpochs: int = Field(80, ge=1, le=500, description="训练总 epoch 数")
    batchSize: int = Field(256, ge=8, le=4096, description="训练批大小")
    ...
    balancedSampling: bool = Field(True, description="是否启用平衡采样")
    samplerAlpha: float = Field(0.75, ge=0, le=2, description="平衡采样强度")
    optimizer: Literal["adamw", "adam", "sgd"] = Field("adamw", description="优化器")
    learningRate: float = Field(3e-4, gt=0, le=1, description="学习率")
    weightDecay: float = Field(2e-4, ge=0, le=1, description="权重衰减")
    scheduler: Literal["cosine_warmup", "reduce_on_plateau", "step", "none"] = Field(
        "cosine_warmup", description="学习率调度器"
    )
    warmupEpochs: int = Field(8, ge=0, le=100, description="Warmup epoch 数")
    minLr: float = Field(5e-6, ge=0, le=1, description="最小学习率")
    ...
    plateauPatience: int = Field(4, ge=1, le=100, description="Plateau patience")
    plateauMinLr: float = Field(5e-6, ge=0, le=1, description="Plateau 最小学习率")
    inputDropout: float = Field(0.05, ge=0, lt=1, description="输入层 Dropout")
    backboneDropout: float = Field(0.20, ge=0, lt=1, description="主干网络 Dropout")
    headDropout: float = Field(0.20, ge=0, lt=1, description="任务头 Dropout")
    headSamples: int = Field(4, ge=1, le=16, description="Multi-sample dropout 次数")
    posWeight: float = Field(-1.0, ge=-1, le=100, description="正样本权重，-1 表示自动计算")
    labelSmoothing: float = Field(0.01, ge=0, le=0.5, description="标签平滑")
    focalGamma: float = Field(2.0, ge=0, le=10, description="Focal loss gamma")
    focalAlpha: float = Field(0.75, ge=0, le=1, description="Focal loss alpha")
    bceWeight: float = Field(1.0, ge=0, le=10, description="BCE 损失权重")
    focalWeight: float = Field(0.35, ge=0, le=10, description="Focal 损失权重")
    earlyStop: bool = Field(True, description="是否启用 Early Stopping")
    patience: int = Field(16, ge=1, le=200, description="Early Stopping patience")
    minDelta: float = Field(5e-4, ge=0, le=1, description="Early Stopping 最小改善阈值")
    earlyStopMetric: Literal["auc", "pr_auc", "loss", "clf_loss", "accuracy", "balanced_accuracy", "f1", "precision", "recall"] = Field(
        "pr_auc", description="Early Stopping 监控指标"
    )
    useAmp: bool = Field(True, description="是否启用 AMP")
    gradClip: float = Field(2.0, ge=0, le=100, description="梯度裁剪阈值")
    ...
    thresholdBeta: float = Field(0.8, gt=0, le=5, description="F-beta 的 beta 参数")
