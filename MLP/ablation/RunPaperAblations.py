...
COMPONENT_BASELINE = ExperimentSpec(
    ...
    description="Component-ablation baseline: residual MLP with LayerNorm, GELU, MLP head, weighted BCE, and the same non-variable defaults as the system MLP.",
)
...
COMPONENT_ABLATIONS = [
    ExperimentSpec(
        ...
        group="components",
        slug="no_residual",
        description="Replace residual blocks with plain feed-forward blocks while keeping all non-variable parameters aligned with the system MLP configuration.",
    ...
    ExperimentSpec(
        ...
        group="components",
        slug="no_norm",
        description="Remove LayerNorm from the input projection, backbone blocks, and classification head while keeping the remaining settings aligned with the system MLP configuration.",
    ...
    ExperimentSpec(
        ...
        description="Keep the architecture unchanged but replace GELU with ReLU while keeping the remaining settings aligned with the system MLP configuration.",
    ...
    ExperimentSpec(
        ...
        group="components",
        slug="linear_head",
        description="Replace the two-layer MLP classification head with a single linear layer while keeping the remaining settings aligned with the system MLP configuration.",
...
STRUCTURE_ABLATIONS = [
    ExperimentSpec(
        key="structure_3layer_small",
        group="structure",
        slug="3layer_small",
        description="Three-layer residual MLP with hidden dimensions (128, 256, 128).",
        model_overrides={"hidden_dims": (128, 256, 128)},
    ),
    ExperimentSpec(
        key="structure_3layer_large",
        group="structure",
        slug="3layer_large",
        description="Three-layer residual MLP with hidden dimensions (256, 512, 256), which is exactly twice the small-width counterpart.",
        model_overrides={"hidden_dims": (256, 512, 256)},
    ),
    ExperimentSpec(
        key="structure_4layer_small",
        group="structure",
        slug="4layer_small",
        description="Four-layer residual MLP with hidden dimensions (128, 256, 128, 64).",
        model_overrides={"hidden_dims": (128, 256, 128, 64)},
    ),
    ExperimentSpec(
        key="structure_4layer_large",
        group="structure",
        slug="4layer_large",
        description="Four-layer residual MLP with hidden dimensions (256, 512, 256, 128), matching the current system MLP width setting.",
        model_overrides={"hidden_dims": (256, 512, 256, 128)},
    ),
    ExperimentSpec(
        key="structure_5layer_small",
        group="structure",
        slug="5layer_small",
        description="Five-layer residual MLP with hidden dimensions (128, 256, 256, 128, 64).",
        model_overrides={"hidden_dims": (128, 256, 256, 128, 64)},
    ),
    ExperimentSpec(
        key="structure_5layer_large",
        group="structure",
        slug="5layer_large",
        description="Five-layer residual MLP with hidden dimensions (256, 512, 512, 256, 128), which is exactly twice the small-width counterpart.",
        model_overrides={"hidden_dims": (256, 512, 512, 256, 128)},
    ),
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Training epochs. Default: inherit from TrainConfig.py",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Learning rate. Default: inherit from TrainConfig.py",
    )
    parser.add_argument(
        "--weight-decay",
        type=float,
    ...
    parser.add_argument(
        ...
        help="F-beta for threshold search",
    )
...
def resolve_patience(spec: ExperimentSpec) -> int:
    return DEFAULT_GLOBAL_PATIENCE
...
def build_runtime_config(
    ...
    *,
    ...
    cfg.train.patience = resolve_patience(spec)
...
def write_markdown_summary(path: Path, summary_rows: list[dict[str, Any]]) -> None:
    ...
    for row in summary_rows:
        lines.append(
            "| {index} | {group} | {name} | {runs} | {f1_mean} +- {f1_std} | {auc_mean} +- {auc_std} | {pr_auc_mean} +- {pr_auc_std} | {precision_mean} +- {precision_std} | {recall_mean} +- {recall_std} | {acc_mean} +- {acc_std} | {bal_mean} +- {bal_std} | {loss_mean} +- {loss_std} | {epochs_mean} +- {epochs_std} | {spe_mean} +- {spe_std} | {dur_mean} +- {dur_std} | {stop_rate} |".format(
