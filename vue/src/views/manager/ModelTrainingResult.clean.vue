  ...
  <div>
    <div class="card hero-card">
      <div>
        ...
        <div class="hero-desc">
          页面集中展示本次训练的测试指标、详细分类报告、实际生效的超参数，以及训练过程图像与权重保存入口。
        </div>
      </div>
      <div class="hero-meta">
    ...
    <div v-if="currentJob" class="card section-card">
      ...
      <el-alert
        v-if="isRunning"
        title="任务仍在执行中，页面会自动刷新状态，训练完成后指标和图像会完整呈现。"
        type="info"
        show-icon
        :closable="false"
    ...
    <template v-if="summary">
      ...
      <div class="card section-card">
        ...
        <div class="section-subtitle">以下内容为本轮训练最终生效的参数配置。</div>
      ...
      <div class="card section-card">
        <div class="section-title">训练图像</div>
        <div class="section-subtitle">以下图像由 Python 在训练结束后生成并保存，前端直接读取任务产物进行展示。</div>
    ...
    <el-empty v-else-if="!loading" description="未查询到训练结果，请返回训练页面重新发起任务。" />
  </div>
</template>
...
const statusText = computed(() => ({ running: '训练中', completed: '已完成', failed: '训练失败' }[currentJob.value?.status] || '加载中'))
const statusTagType = computed(() => ({ running: 'warning', completed: 'success', failed: 'danger' }[currentJob.value?.status] || 'info'))
...
const progressPercent = computed(() => Math.min(100, Math.max(0, Number((Number(currentJob.value?.progress || 0) * 100).toFixed(1)))))
const progressBarStatus = computed(() => currentJob.value?.status === 'failed' ? 'exception' : currentJob.value?.status === 'completed' ? 'success' : '')
...
const formatMetric = (value, digits = 4) => value === undefined || value === null || value === '' ? '-' : Number(value).toFixed(digits)
const formatPercent = (value) => value === undefined || value === null || value === '' ? '-' : `${(Number(value) * 100).toFixed(2)}%`
